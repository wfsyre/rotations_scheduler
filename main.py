...
class Rotations:
    ...
    def __init__(self, df: pd.DataFrame, in_format='%m/%d/%Y', out_format='%m/%d'):
        dates = [col.split("-") for col in df.columns[1:]]
        names = df[df.columns[0]].to_list()[1:]
        for index, name in enumerate(names):
            self.name_to_number[name] = index
            self.names.append(name)
        self.in_format = in_format
        data_format = []
        for date_from, date_to in dates:
            # convert from string format to datetime format
            col_name = date_from + "-" + date_to
            data_format.append([x.strip() for x in df[col_name].to_list()])
            from_obj = datetime.datetime.strptime(date_from + "/" + str(start_year), self.in_format)
            to_obj = datetime.datetime.strptime(date_to + "/" + str(start_year), self.in_format)
            if self.start_date is not None:
                if from_obj < self.start_date:
                    from_obj = datetime.datetime.strptime(date_from + "/" + str(start_year + 1), self.in_format)
                if to_obj < self.start_date:
                    to_obj = datetime.datetime.strptime(date_to + "/" + str(start_year + 1), self.in_format)
            self.periods.append([from_obj, to_obj])
            if self.start_date is None:
                self.start_date = from_obj
            self.end_date = to_obj
        data_format = np.array(data_format).transpose()
        self.rotations_df = pd.DataFrame(data_format,
                                         columns=[start for start, finish in self.periods],
                                         index=[df[df.columns[0]]])

    def get_rotation(self, date, name):
        range_start = self.get_starting_date(date)
        return self.rotations_df.loc[name][range_start].values[0]

    def get_starting_date(self, date):
        range_start = None
        for col in self.rotations_df.columns:
    ...
    def get_valid_names_for_date(self, date, excluded_rotations):
        range_start = self.get_starting_date(date)
        series = self.rotations_df[range_start]
        for ex in excluded_rotations:
            series = series[series != ex]
        list_series = [x[0] for x in series.index.to_list()[1:]]
        return [self.name_to_number[n] for n in list_series]
...
def get_cover_days(from_date, to_date):
    dates = []
    pairs = []
    cal = USFederalHolidayCalendar()
    holidays = cal.holidays(start=from_date, end=to_date).to_pydatetime()
    covered = set()
    for holiday in holidays:
        ...
        if holiday.weekday() == 4:
            ...
            days = [day for day in days if from_date <= day <= to_date]
            covered.update(days)
            ...
            # holiday is on its own
            days = [holiday]
            covered.update(days)
            pairs.append(tuple(days))
            dates += days

    d = from_date  # first day in range
    d += datetime.timedelta(days=6 - d.weekday())  # First Sunday
    while from_date <= d <= to_date:
        if d not in covered:
            yesterday = d + datetime.timedelta(days=-1)
            if from_date <= yesterday <= to_date:
...
def get_schedule_csp(rotations):
    problem = Problem()
    dates, pairs = get_cover_days(rotations.start_date, rotations.end_date)
...
def get_viable_names_for_date(date, rotations):
    names = rotations.get_valid_names_for_date(date, ["NF"])
    return names


def get_schedule_genetic(rotations, mutation_percent_genes=5, num_generations=2000, sol_per_pop=100):
    ...
    for pair in pairs:
        ...
        count += 1
    solution_variable_to_name = {rotations.name_to_number[name]: name for name in rotations.name_to_number.keys()}
    ...
    while solution_index < len(solution):
        if solution[solution_index] == -1:
            ...
            primary = solution_variable_to_name[solution[solution_index]]
            secondary = solution_variable_to_name[solution[solution_index + 1]] if solution[solution_index + 1] != -2 else ""
...
def fitness_function(ga_instance, solution, solution_idx):
            ...
            for j in range(start_index, end_index):
                if backup:
                    if solution[j] == -2:
                        ...
                        rotation = rotations.get_rotation(date, name)
                        if rotation == "NF":
    ...
    for name in rotations.name_to_number.keys():
        if rotations.name_to_number[name] not in val:
            min_value = 0
...
if __name__ == '__main__':
    ...
    t_start = time.time()
    rotations = read_rotations("real_schedule.csv")
    sol, fitness = get_schedule_genetic(rotations)
    solution_to_csv(sol, rotations)
    t_end = time.time()
    print("Finished execution in", t_end - t_start, "seconds")
